...
from CustomCityLearn.citylearn.data import EnergySimulation, CarbonIntensity, Pricing, Weather
from CustomCityLearn.citylearn.gen_energy_model import Battery, PV
from CustomCityLearn.citylearn.preprocessing import Encoder, PeriodicNormalization, OnehotEncoding, RemoveFeature, Normalize, NoNormalization
...
class Building():
    ...
    def reset(self):#, temp_random, hum_random):
        ...
        self.__non_shiftable_load = (self.__humidity-60)/20/self.ac_efficiency + (30-self.__outdoor_dry_bulb_temperature)/25/self.ac_efficiency + self.__non_shiftable_load_base[:self.__humidity.size]
        self.__solar_generation = self.pv.get_generation((self.__outdoor_dry_bulb_temperature*self.solar_efficiency*self.panel**2) * self.__solar_generation_base[:self.__outdoor_dry_bulb_temperature.size]) * -1
        
        self.__net_electricity_consumption = []
        self.__net_electricity_consumption_price = []
        
        a = self.__non_shiftable_load
        b = self.__solar_generation
