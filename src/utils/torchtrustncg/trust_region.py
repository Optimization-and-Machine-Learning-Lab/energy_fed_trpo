...
class TrustRegion(optim.Optimizer):
    ...
    def _compute_hessian_vector_product(
        ...
        return torch.cat([torch.flatten(vp) for vp in hess_vp if vp is not None], dim=-1)
        #  hess_vp = torch.cat(
        #  [torch.flatten(vp) for vp in hess_vp], dim=-1)
        #  return torch.flatten(hess_vp)

    @torch.enable_grad()
    def _compute_fisher_vector_product(
            self,
            closure: Callable,
            p: Tensor) -> Tensor:
    ...
    def _improvement_ratio(self, p, start_loss, gradient, closure):
        ...
        hess_vp = self._compute_fisher_vector_product(closure, p)
        ...
        # new_loss, _, _ = closure(backward=False)
    ...
    def _solve_subproblem_cg(
        ...
        while True:
            ...
            #     flat_grad, direction
            # )
            hessian_vec_prod = self._compute_fisher_vector_product(
                closure, direction
            )
    ...
    def _solve_subproblem_krylov(
        ...
        while True:
            ...
            Hp = self._compute_fisher_vector_product(closure, p)
            ptHp = torch.sum(Hp * p)
    ...
    def step(self, closure=None) -> float:
        ...
        starting_loss = loss_module(data_sample)['loss_objective']
        ...
        # starting_loss = loss_module(data_sample)['objective_loss']

        # kl_grad = torch.autograd.grad(fn_kl(data_sample)[2].mean(), self._params, create_graph=True)
        ...
        if self.opt_method == 'cg':
            param_step, hit_boundary = self._solve_subproblem_cg(
                starting_loss, flat_grad, closure, trust_radius)
        else:
