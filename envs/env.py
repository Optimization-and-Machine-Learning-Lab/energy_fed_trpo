...
class BanmaEnv(Env):
    def __init__(self, batch_size=1, n_actions=1, action_diff_penalty=0.0):
        super(BanmaEnv, self).__init__()
        self.dim_action = n_actions
        # self.solar_gen_coef = 120.8983

        self.time = 0
        self.end_time_step = 24
        self.price_coef = 1.0
        self.emission_coef = 1.0
        ...
        self.daily_solar, self.daily_load, self.carbon, self.price = self.__load()
        self.storage = 0.0
        self.capacity = 1.0

        # self.baseline_price = -3.8723834906814907       # TODO: remove this

    def __load(self):
        # weather = pd.read_csv("/home/yunxiang.li/citylearn/banma_citylearn/citylearn_challenge_2022_phase_1/weather.csv")
        # diffuse_solar = weather[["Diffuse Solar Radiation [W/m2]"]].to_numpy().squeeze()
        # direct_solar = weather[["Direct Solar Radiation [W/m2]"]].to_numpy().squeeze()
        # out_temp = weather[["Outdoor Drybulb Temperature [C]"]]
        # out_hum = weather[["Relative Humidity [%]"]]
        # diffuse_solar /= np.linalg.norm(diffuse_solar)
        # direct_solar /= np.linalg.norm(direct_solar)

        # price_csv = pd.read_csv("/home/yunxiang.li/citylearn/banma_citylearn/citylearn_challenge_2022_phase_1/pricing.csv")
        # price = price_csv[["Electricity Pricing [$]"]].to_numpy().squeeze()

        # carbon_csv = pd.read_csv("/home/yunxiang.li/citylearn/banma_citylearn/citylearn_challenge_2022_phase_1/carbon_intensity.csv")
        # carbon = carbon_csv[["kg_CO2/kWh"]].to_numpy().squeeze()
        ...
        carbon = np.array([0.13457588, 0.13031782, 0.12807456, 0.12726278, 0.12840281, 0.13472049,
                           0.14503678, 0.15087238, 0.15623535, 0.16155268, 0.16493331, 0.16717851,
                           0.16792708, 0.16871218, 0.16950457, 0.17114729, 0.17467535, 0.17835915,
                           0.17930293, 0.1777637 , 0.17537895, 0.16805733, 0.15326134, 0.14348394])
        ...
        return daily_solar, daily_load, carbon, price

    def reset(self):
        self.time = 0
        self.storage = 0.0      # TODO: always set to 0.0???
    ...
    def step(self, action):
        ...
        baseline_price = self.price[(self.time-1)%24] * net_con #max(net_con, 0.0)
        baseline_emission = self.carbon[(self.time-1)%24] * net_con #max(net_con, 0.0)
        ...
        if action > 0.0:                            # charge
            ...
            valid_action = 0.0
        # print("valid action", valid_action, ", storage", self.storage, ", net_con without battery", net_con, ", net_con with battery", net_con + valid_action)
        self.storage += valid_action
        net_con += valid_action
        net_con = max(net_con, 0)
        # grid_con = max(0.0, net_con)                # electricity from the grid

        price_reward = self.price[(self.time-1)%24] * net_con #grid_con
        emission_reward = self.carbon[(self.time-1)%24] * net_con #grid_con
        
        # print("time", self.time, "net_con", net_con)
        # print(price_reward, emission_reward, baseline_price, baseline_emission)
        # reward = self.price_coef * price_reward + self.emission_coef * emission_reward
        
        price_ratio = price_reward / baseline_price
        emission_ratio = emission_reward / baseline_emission
        action_diff = (action - valid_action) ** 2

        # reward = - price_ratio - self.action_diff_penalty * action_diff
        ...
        return np.array(state), reward, done, {"price_ratio": price_ratio, "emission_ratio": emission_ratio, "hour": self.time, "valid_action": valid_action, "action_diff": action_diff, "ratio": price_ratio, "price_reward": -price_reward}
